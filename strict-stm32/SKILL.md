---
name: strict-stm32
description: STM32 嵌入式开发的强约束工作流 skill。基于"CubeMX 一次性产骨架 + AI 增量演进"架构，反幻觉（写代码前必读 HAL/CMSIS 头文件不凭记忆）、对话式询问关键硬件决策、HARDWARE.md 作为真相源强制同步。任何 STM32 工程需要添加外设（USART/I2C/SPI/ADC/TIMER/DMA/CAN/USB/QSPI）、写 driver/bsp/components/app 代码、修改硬件配置、集成传感器或显示器芯片时都应使用本 skill。即使用户没说"用 skill"，只要任务涉及 STM32 外设驱动、传感器 BSP、应用层业务逻辑实现，都应触发本 skill。新手配置第一个 LED 闪烁、老手做陀螺仪姿态解算，都走同一套工作流。
---
...
- **`project_root/code/`** —— CubeMX 工程目录（= `.ioc` 所在目录）= VSCode 工作区根 = CMake 项目根。**所有 AI 写的代码和文档都加在这里**（driver/bsp/components/app/sandbox/hw/tasks/HARDWARE.md/PROJECT.md）。`code/` 是 skill 文档里的占位名，用户实际项目可以叫 `cs/` `myapp/` 等 —— 但 driver/bsp/components/app 等 AI 加的内容**永远在 code/ 里面**，不在 project_root 下与 code/ 平级
- **`project_root/code/Core/Inc`、`Core/Src`、`Drivers`、`cmake/`** —— CubeMX 6.10+ 的内部分层

**为什么 AI 加的内容放进 code/ 里**：

1. **VSCode 工作区一致性**：CMakeLists.txt 在 `code/`，用户用 VSCode 打开 `code/` 作为工作区根时，IntelliSense 才能索引到 driver/bsp/components/app 下的代码。如果放在 project_root 下与 code/ 平级，VSCode 打开 code/ 就看不到它们
2. **CMake 相对路径简化**：driver/bsp/components/app 在 code/ 下，CMake 直接写 `driver/driver_led.c`，不用 `../driver/driver_led.c`
3. **项目结构紧凑**：所有项目相关的东西在 code/ 里，project_root 只剩 `.claude/`，干净

**代价**：失去了"AI 代码与 CubeMX 产物的物理隔离"。但 CubeMX 重新生成时只动它自己生成的文件（`Core/` `Drivers/` `<project>.ioc` 等），不会动 AI 加的 `code/driver/` `code/bsp/` `code/components/` 这些目录 —— 实践中误伤风险很低。
...
└── code/                           # CubeMX 工程目录（= .ioc 所在目录）= VSCode 工作区根
    ...
    ├── components/                 # AI 加（正式：纯软件中间件，如 cmp_ring_buffer.c）
...
2. 在 `code/` 下创建目录骨架：`driver/ bsp/ components/(含 protocol/ utils/ 子目录) app/ sandbox/ hw/datasheets/ tasks/`（**7 个目录必须同时建**，即使 `bsp/` `components/` 等暂时为空也要建占位，SR13）
...
| "加 / 实现 / 写一个 XX 驱动 / 功能" | 正式功能 | `driver/` `bsp/` `components/` `app/` |
...
写任何 `driver/` `bsp/` 代码前，**必须**先 Read（写 `components/` 纯逻辑代码**不需要**读 HAL —— components 不依赖硬件，其真值源是协议规范，见下方说明）：
...
写 `components/protocol/`（协议解析）代码前，真值源是**协议规范**而非 HAL：
...
- `driver/` `bsp/` `components/` `app/` 自由写（创建新文件，components 文件用 `cmp_` 前缀，如 `cmp_ring_buffer.c`）
...
- 跨层 include 单向：`app → components → bsp → driver`，反向禁止
- 共享外设（如 I2C1）单例：`bsp/` 层禁止直接碰 `hi2c1` 句柄，必须通过 `driver_i2c_*()` API
- **Components 纯逻辑**：`components/` 层严禁 include HAL/CMSIS、`driver/`、`bsp/` 头文件；只接收 Byte 输入、输出 Struct，不触碰硬件句柄与业务逻辑；如需要把帧发给串口，由 app 层注册回调
...
- 加/删了 `code/driver/bsp/components/app/sandbox/` 下 `.c` 文件 → "需要重跑 `cmake --preset Debug && cmake --build build/Debug` 才会让文件列表生效"
...
写 `code/driver/bsp/components/app/` 代码时必须遵守两条风格规则。和架构约束（SR1–SR10、SR13）不同，这两条是**可读性**约束，但同样强制。
...
| SR4 | 写 `driver/` 或 `bsp/` 前必须先 Read `code/Drivers/` 下对应 HAL/CMSIS 头文件，基于真值写代码（components 除外：纯逻辑层，真值源是协议/帧格式定义，不读 HAL） |
| SR5 | `bsp/` 层禁止直接碰外设句柄（`hi2c1` / `huart2` / ...），必须通过 `driver/` 层 API |
| SR6 | include 单向：`app → components → bsp → driver`。components 不允许 include bsp/driver/app；bsp 不允许 include components/app；driver 不允许 include 上层任何一层 |
| SR7 | 任何修改硬件配置的任务，结束前必须同步更新 `code/HARDWARE.md`。不更新视为任务未完成 |
| SR8 | 加/删 `code/driver/bsp/components/app/sandbox/` 下 `.c` 文件后，必须显式提示用户在 `code/` 下重跑 `cmake --preset Debug && cmake --build build/Debug` |
...
| SR13 | 项目代码必须按 `app/components/bsp/driver` 四层组织。项目初始化时四个目录必须同时建（`components/` 内含 `protocol/` `utils/` 子目录，即使暂时空着也要建占位）。业务代码不许跳过 `components/` `bsp/` 直接 driver ↔ app —— 即使当前只有 LED/UART 这种 MCU 内部外设，也要走 `app → driver`，等接外部芯片或协议时 `bsp/` `components/` 自然填上，分层骨架一开始就立起来 |
