...
4. Project Manager → Project：
   ...
   - 把生成路径设到 <project_root>/code/（未来的 driver/bsp/components/app 等会建在 code/ 里）
5. 点 GENERATE CODE
...
```bash
mkdir -p driver bsp components components/protocol components/utils app sandbox hw/datasheets tasks
```
...
按 `references/cmake.md` 的说明，在顶层 `target_sources` 钩子追加 `driver/*.c` `components/**/*.c` 等源文件（不带 `../`），在 `target_include_directories` 钩子追加 `driver bsp components components/protocol components/utils app sandbox` 等 include path。**不要动** `add_subdirectory(cmake/stm32cubemx)` 和其他 CubeMX 自动生成的部分。
...
- project_root/             你的工作目录
  ...
  - code/                   CubeMX 工程 = VSCode 工作区根
    ...
    - driver/bsp/components/app/  正式代码层（components/ 内含 protocol/ utils/ 子目录，空，按需添加）
    - sandbox/              脚手架代码（空）
...
```bash
Glob driver/*.c bsp/*.c components/**/*.c app/*.c sandbox/*.c
```
