...
5. 生成路径设到 `project_root/code/`（driver/bsp/components/app 等 AI 加的内容会建在 code/ 里）
...
**物理位置**：app/bsp/components/driver/sandbox 都在 `code/` 下（与 `Core/` `Drivers/` 平级）；`code/` 是 CubeMX 工程目录 = VSCode 工作区根。CMake 里 driver 源文件路径直接写作 `driver/foo.c`，详见 `references/cmake.md`。
...
| `components/` | 纯软件中间件：算法、通信协议栈、数据结构、系统服务 | `components/protocol/cmp_frame_parser.c`、`components/utils/cmp_ring_buffer.c` |
...
`components/` 是四层架构新增的纯软件层，分两个子目录：

```
components/
├── protocol/       报文切帧、CRC 校验、Modbus/自定义协议解包（如 cmp_modbus.c、cmp_frame_parser.c）
└── utils/          RingBuffer、FIFO、CRC 算法、调试日志（如 cmp_ring_buffer.c、cmp_crc16.c）
```
...
- 如果中间件需要把数据送出去（如把组装好的帧发给串口），通过 **app 层注册回调函数**实现，不在 components 里直接调用驱动

**为什么加这一层**：

- 协议解析（帧头 `0xAA 0x55`、CRC 校验、长度字段）是纯逻辑，与具体串口/总线无关 —— 放进 components 可以在 PC 上单测，也可以在换 MCU 时原样复用
- 把"字节级拆包"从 app 层剥离后，app 只消费合法 Struct，业务代码干净且不重复实现协议
- 与 driver/bsp 一样实现"底层不知道上层存在"：components 不知道数据从哪来、到哪里去

### 分层规则

- `app/` 可以 include `bsp/`、`components/` 和 `driver/`（但通常通过 bsp/components 间接用 driver）
- `components/` 可以 include 同层头文件（`utils/` 与 `protocol/` 互用），**不允许** include `bsp/`、`driver/`、`app/` 及 HAL/CMSIS
- `bsp/` 可以 include `driver/`，**不允许** include `components/` 或 `app/`
- `driver/` **不允许** include `bsp/`、`components/` 或 `app/`
- 任何层都可以 include `code/Drivers/` 的 HAL 头文件（components 除外，见上）

**为什么这么分**：

- 让"通用驱动"（driver）可以跨项目复用 —— 下个项目换个 MCU，driver_i2c.c 改改就能用
- 让"外设芯片驱动"（bsp）独立于应用 —— MPU6050 驱动既能给姿态解算用，也能给数据记录用
- 让"协议与算法"（components）完全独立于硬件 —— 帧解析、CRC 在 PC 上也能测，换 MCU 原样复用
- 让"业务逻辑"（app）独立于硬件细节 —— 改 MPU6050 为 ICM42688 时，app 层 API 不变；改协议时只动 components
...
CubeMX 生成的 CMake 主入口。AI 在顶层 `target_sources` 钩子里追加 `driver/bsp/components/app/sandbox/` 下的源文件（直接写 `driver/foo.c`，不带 `../`），在 `target_include_directories` 里追加 `driver bsp components app sandbox` 等 include path。
...
短期看省事，但 `main.c` 必然膨胀成屎山。所有业务代码进 `driver/bsp/components/app/`，`main.c` 只做"装配"（init 调用 + 主循环 step 调用）。这样 `main.c` 永远干净，一眼能看出"这个项目做了什么"。
...
app ──include──→ components（components 内部互用，不 include 任何硬件层）
```

**禁止反向**：

- `components/` 不能 include `bsp/`、`driver/`、`app/`（纯逻辑，不感知硬件）
- `bsp/` 不能 include `components/` 或 `app/`
- `driver/` 不能 include `components/`、`bsp/` 或 `app/`
