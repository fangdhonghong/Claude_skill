...
3. [加 driver/bsp/app/sandbox 源文件](#加-driverbspappsandbox-源文件)
...
`code/` 是 CubeMX 工程目录名（用户也可以用项目名如 `cs/`），driver/bsp/app 永远在 `code/` **里面**（与 Core/ Drivers/ 平级）。从 `code/CMakeLists.txt` 看 driver 是 `driver`（不带 `../`）。
...
- AI 不动它的话，`driver/bsp/app/` 下的代码加不进编译
...
## 加 driver/bsp/app/sandbox 源文件

**推荐做法**：在顶层 `code/CMakeLists.txt` 的 `target_sources` 钩子里**显式**加源文件路径。driver/bsp/app 在 `code/` 里，路径直接写 `driver/foo.c`（不带 `../`）。
...
> target_sources(${CMAKE_PROJECT_NAME} PRIVATE ${DRIVER_SRC} ${BSP_SRC} ${APP_SRC} ${SANDBOX_SRC})
> ```
>
...
在顶层 `code/CMakeLists.txt` 的 `target_include_directories` 钩子里加（路径不带 `../`，driver/bsp/app 在 code/ 下）：
...
- 在 `driver/bsp/app/sandbox/` 下**新增** `.c` 文件
- 在 `driver/bsp/app/sandbox/` 下**删除** `.c` 文件
- 在 `driver/bsp/app/sandbox/` 下**重命名** `.c` 文件
